...
if len(A_num) != N or N == 0:
    ...
    for i in range(1, N):
        ...
        if count < min:
            min = count
            index = i
    print(f'Число {A_num[index]} в списке A наиболее близко по величине к числу {X}, разница составляет {abs(X - A_num[index])}')
...
rus = {1:'АВЕИНОРСТ',
      	...
      	8:'ШЭЮ',
      	10:'ФЩЪ'}
N = abs(int(input('Введите 1, если играем в английском варианте, либо 0, если в русском варианте: ')))
word = input('Введите слово: ').upper()
if N == 1:
	print('Ваша стоимость составит', sum([k for i in word for k, v in eng.items() if i in v]), 'очков')
elif N == 0:
	print('Ваша стоимость составит', sum([k for i in word for k, v in rus.items() if i in v]), 'очков')
else:
    print('Ошибка')